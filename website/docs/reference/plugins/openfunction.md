...
| Code | Status | When |
|---|---|---|
| `OPENFUNCTION_CALLOUT_ERROR` | 504 | The function callout exceeded `timeout`. |
| `OPENFUNCTION_CALLOUT_ERROR` | 503 | Connecting to or exchanging with the endpoint failed. |
| `OPENFUNCTION_CALLOUT_ERROR` | 502 | The request could not be built (e.g. an invalid endpoint URI). |
