...
| E2E-SN-05 | Named output ports route end-to-end | A supernode with two `type: output` boundaries (`output` and `blocked`) wraps a `condition` node branching on header presence, plus a second `type: error` boundary (`oops`) with no inner edges; a policy wires `gate.success -> up.in` (echo upstream) and `gate.blocked -> deny.in` (a `response-rewrite` returning a fixed `403`), leaving `gate.oops` unwired. A request with the header reaches the echo backend (`200`); without it, the fixed `403` deny response; the save with `gate.oops` unwired succeeds — error-kind instance ports stay optional-wiring even with more than one declared; saving the policy with `gate.blocked` left unwired is rejected (`4xx`) with a message containing `output port 'blocked' of supernode instance` and naming `gate.blocked` — mandatory wiring surfaces through the Admin API — and the prior good policy keeps serving in the meantime |
| E2E-SN-06 | Extract selection as supernode | **Browser.** On a policy `listener -> mid1 -> mid2 -> up -> client`, select `mid1` and `mid2` (click + Ctrl-click — `@xyflow/react`'s default `multiSelectionKeyCode` on non-mac), click the toolbar's "Extract Supernode" button, name it in the dialog, confirm. The new supernode appears in the sidebar library; the canvas now shows one node labeled `⬡ <name>` in `mid1`/`mid2`'s place; Save Policy succeeds (no "Failed to save policy" toast); a data-plane request through the route still round-trips to the echo backend |
...
| E2E-PC-02 | **Browser.** Open a route whose node references a shared config with a local `response_status` override, click the node, edit the override back to the inherited value, then toggle a key the shared config does not set, and Save Policy | The inspector's form shows the local override flagged "overrides shared" and the inherited body **inline in its own field** (not only in a read-only blob) unflagged. Editing the override to equal the inherited value clears the flag (the key auto-drops from local config); the toggled key is flagged "added". The saved policy's node config contains only the added key — the equal edit was not persisted |
| E2E-PC-03 | **Browser.** Click a node holding a plain inline config, use "Save as shared config" (name + submit), then Save Policy and re-request the route; finally retry the dialog with the same name | The shared config is created with the node's effective config; the inspector's picker switches to it with no override flags; the saved policy node carries `config_ref` and an empty local config; live traffic is byte-identical to before the extraction. The duplicate name is rejected client-side with an "already exists" error (the PUT is an upsert, so the UI must guard) |
...
| E2E-STORE-10 | *Gated on `FEATHERBIT_TEST_REDIS_URL`, same convention as `E2E-SESS-*`.* `GET /store-kv/counter?id=<fresh-uuid>` against a route whose policy runs `store-get` (`key: retry:{{request.query.id}}`) and wires its `miss` port to a distinct response from `success` | A never-written `id` takes the `miss` port (`{"seen": false}`); after a `store-incr` on the same key (E2E-STORE-11), the identical request instead takes `success` (`{"seen": true, "count": <n>}`) — proving `miss` and a real value are reachable from the same node in the same route, not just asserted separately in unit tests |
| E2E-STORE-11 | *Gated.* Four `POST /store-kv/incr?id=<fresh-uuid>` in a row against a policy running `store-incr` (`ttl_seconds: 5`) followed by a `condition` node capping at `2` | Responses `count: 1`, `count: 2`, then `429` (over the cap) for the third; after sleeping past the 5s TTL, the fourth resets to `count: 1` — the live-redis analogue of `test_incr_does_not_refresh_the_ttl`, but observed through a route instead of a unit test |
| E2E-STORE-12 | *Gated.* `POST /store-kv/incr?id=<fresh-uuid>`, then `DELETE /store-kv/counter?id=<fresh-uuid>` (`store-delete`), then `GET /store-kv/counter?id=<fresh-uuid>` again | The final `GET` takes `store-get`'s `miss` port exactly as a never-written key would — the delete leaves no trace, round-tripping `store-delete` against a key that actually existed (E2E-STORE-04 only covers deleting one that never did) |
| E2E-STORE-13 | *Gated.* `POST /api/policies/validate` a policy running `store-get`, `store-incr` and `store-delete` ahead of `upstream`, then a live request through the equivalent route | The `buffering` report (same shape as `E2E-STREAM-02`) never names any of the three `store-*` nodes as a `blocked_by`, and the live request still reaches the upstream — the nodes' `reads_response_body()` is false unless their `key`/`value` template references `{{response.body}}`, so by default they must not force a streaming-eligible route onto the buffered path |
