...
| E2E-SN-05 | Named output ports route end-to-end | A supernode with two `type: output` boundaries (`output` and `blocked`) wraps a `condition` node branching on header presence; a policy wires `gate.success -> up.in` (echo upstream) and `gate.blocked -> deny.in` (a `response-rewrite` returning a fixed `403`). A request with the header reaches the echo backend (`200`); without it, the fixed `403` deny response; saving the policy with `gate.blocked` left unwired is rejected (`4xx`) with a message containing `output port 'blocked' of supernode instance` and naming `gate.blocked` — mandatory wiring surfaces through the Admin API — and the prior good policy keeps serving in the meantime |
