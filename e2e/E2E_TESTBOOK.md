...
| E2E-UI-26 | Open `rt-policy`'s upstream node; type a half placeholder, then `${ECHO_PORT}` into Target 1 Port; save and request `/rt/…` | The seeded `${ECHO_PORT:-3010}` is shown (a number input showed it empty); the partial text is flagged `aria-invalid` and not stored; the whole placeholder is saved as that string and the route still answers `200` once it resolves at compile time |
...
| E2E-PC-04 | **Browser.** Open a shared `error-handler` config from the Plugin configs library, type `{{env.LOG` in its body template, then set the status code to `${E2E_STATUS:-503}` and save | The suggestion popover lists `env.LOG_LEVEL` (the shared config editor used to get no suggestions at all); the placeholder status code is stored as that string |
