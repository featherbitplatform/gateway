...
//! - `body` renders a `{{namespace.path}}` template (plus legacy `$var`
//!   interpolation, including `$status`) as the new body,
//!
...
/// the body with an interpolated `{{namespace.path}}` (plus legacy `$var`)
/// template — but only when the response was generated by the gateway
/// (`Context.errors` non-empty), unless `always` is set. This plugin never
