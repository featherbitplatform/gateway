...
mod tests {
    ...
    async fn test_session_mode_code_exchange_redirects_with_cookie() {
        ...
        config.insert("access_token_url".to_string(), serde_json::json!(token_url));
        config.insert("userinfo_url".to_string(), serde_json::json!(userinfo_url));
        config.insert(
            "redirect_uri".to_string(),
            serde_json::json!("https://login.example.com/start"),
        );
        ...
        assert!(set_cookie.starts_with("feishu_session="), "{set_cookie}");
