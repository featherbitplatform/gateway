...
pub async fn call(state: &SharedState, name: &str, a: JsonObject) -> Result<Value, ToolError> {
    match name {
        ...
        "put_route_order" => writes::put_route_order(state, args(a)?).await,
...
static TOOLS: [ToolDef; 35] = [
    ...
    ToolDef { name: "put_route_order", scope: Write, description: "Change route priority. Routes are matched in order and the first match wins, so a more specific route must come before a broader one (e.g. /api/admin/* before /api/*). Args: {\"order\": [\"admin\", \"api\", \"catch-all\"], \"dry_run\": false} — list_routes gives the current order; `order` must name every route exactly once. dry_run supported; live immediately on success.", input_schema: schema_of::<writes::RouteOrderArgs> },
