...
- Field attributes: `template?: 'full' | 'env-only' | 'none'` — **default `'env-only'` for text-like fields** (`text`/`textarea`/`list`/`objects`-text; `'none'` for structural types — `radio`/`select`/`switch`/`number`, which never render `VarInput`). `'full'` is set explicitly, one field at a time, on exactly the fields the Rust plugin renders through `Template` at request time (verified against the plugin's source per field); every other text-like field defaults to `'env-only'`, since offering full request/response/message suggestions on a field the engine never re-renders per request would be dishonest — the suggestion would insert but never resolve. (Ruling: a Task 9 review found the original `default 'full'` policy suggested live context in ~15 plugin families the engine never templates; the human partner ruled context suggestions belong only where fields are genuinely engine-templated, hence the flip.) The flag replaces `vars: true`, whose fields become explicit `'full'` + `legacyDollar: true`.
